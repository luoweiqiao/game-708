...
bool CGameFishingTable::LeaveTable(CGamePlayer* pPlayer, bool bNotify) {
	...
	if (pPlayer != NULL)
	{
		...
		LOG_DEBUG("LeaveTable - roomid:%d tableid:%d chairID:%d uid:%d table_score:%lld", GetRoomID(), GetTableID(), chairID, pPlayer->GetUID(), m_lTableScore[chairID]);
...
{
	LOG_DEBUG("roomid:%d tableid:%d seat_id:%d", GetRoomID(), GetTableID(), seat_id);
	...
	else
	{
		...
		if (pPlayer != NULL)
		{
			...
			LOG_DEBUG("push user roomid:%d tableid:%d seat_id:%d", GetRoomID(), GetTableID(), seat_id);
			m_blingLog.users.push_back(user);
		}		
	}	
	m_operLog.clear();
