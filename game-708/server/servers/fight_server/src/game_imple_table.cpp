...
void CGameFightTable::GetTableFaceInfo(net::table_face_info* pInfo)
{
	...
	{
		pfight->add_chip(m_chip_range[i]);
	}
}
...
bool CGameFightTable::ReAnalysisParam() {
	...
	LOG_DEBUG("reader_json -  roomid:%d,tableid:%d,m_sysBankerWinPro:%d,m_iArrDispatchCardPro:%d %d %d",
...
int64    CGameFightTable::CalcPlayerInfo(uint32 uid,int64 winScore,int64 OnlywinScore,bool isBanker)
{
	...
	else if (iProIndex == Pro_Index_Tiger)
	{
		...
		else if (cbOneValue == cbTwoValue)
		{
			...
			else if (cbTwoValue == 13)
			{
				...
				m_cbTableCardArray[0][0] = g_RandGen.RandRange(1, 12) + cbTwoColor;
			}
			else
			{
				cbCardValue = g_RandGen.RandRange(1, 13);
			}
			m_cbTableCardArray[1][0] = cbCardValue + cbOneColor;
		}
		else
		{
			m_GameLogic.RandCardList(m_cbTableCardArray[0], sizeof(m_cbTableCardArray) / sizeof(m_cbTableCardArray[0][0]));
...
// 福利控制
bool CGameFightTable::DosWelfareCtrl()
{
	...
	LOG_DEBUG("roomid:%d,tableid:%d,sysBankerWinPro:%d,bIsFalgControl:%d,needChangeCard:%d,bIsSysWinScore:%d,cbCardData:0x%02X 0x%02X",
		GetRoomID(), GetTableID(), sysBankerWinPro, bIsFalgControl, needChangeCard, bIsSysWinScore, m_cbTableCardArray[0][0], m_cbTableCardArray[1][0]);
...
bool CGameFightTable::NotWelfareCtrl()
{
	...
	LOG_DEBUG("roomid:%d,tableid:%d,m_sysBankerWinPro:%d,bIsFalgControl:%d,needChangeCard:%d,bIsSysWinScore:%d,isAwCtrl:%d,bIsStockControl:%d,cbCardData:0x%02X 0x%02X",
		GetRoomID(), GetTableID(), m_sysBankerWinPro, bIsFalgControl, needChangeCard, bIsSysWinScore, isAwCtrl, bIsStockControl, m_cbTableCardArray[0][0], m_cbTableCardArray[1][0]);

	return true;
}


//bool CGameFightTable::KilledPlayerCtrl()
//{
//	bool bIsFalgControl = ProgressControlPalyer();
//
...
//	}
//
//	LOG_DEBUG("roomid:%d,tableid:%d,bIsFalgControl:%d,needChangeCard:%d,bIsSysWinScore:%d,cbCardData:0x%02X 0x%02X",
//		GetRoomID(), GetTableID(), bIsFalgControl, needChangeCard, bIsSysWinScore, m_cbTableCardArray[0][0], m_cbTableCardArray[1][0]);
//
//	return true;
//}
...
bool    CGameFightTable::DispatchTableCard()
{
	...
	if (!bIsAreaCtrl && m_pHostRoom != NULL)
	{
		newroom = m_pHostRoom->GetNoviceWelfare();
	}

	if (!bIsAreaCtrl && newroom == 1)
	{
	...
	LOG_DEBUG("dos_wel_ctrl - roomid:%d,tableid:%d,bIsAreaCtrl:%d, newroom:%d, bHaveNotNovicePlayer:%d,ChessWelfare:%d,GetIsAllRobotOrPlayerJetton:%d",
		GetRoomID(), GetTableID(), bIsAreaCtrl, newroom, bHaveNotNovicePlayer, GetChessWelfare(), GetIsAllRobotOrPlayerJetton());
...
bool CGameFightTable::ActiveWelfareCtrl()
{
...
bool    CGameFightTable::SetControlPalyerWinForAW(uint32 control_uid, int64 max_win)
{
...
void CGameFightTable::GetGamePlayLogInfo(net::msg_game_play_log* pInfo)
{
	net::msg_fight_play_log_rep* pplay = pInfo->mutable_fight();
	for (uint16 i = 0; i<m_vecRecord.size(); ++i)
	{
		net::fight_play_log* plog = pplay->add_logs();
		fightGameRecord& record = m_vecRecord[i];
		for (uint16 j = 0; j<JETTON_INDEX_COUNT; j++) {
			plog->add_seats_win(record.wins[j]);
		}
...
void CGameFightTable::GetGameEndLogInfo(net::msg_game_play_log* pInfo)
{
	net::msg_fight_play_log_rep* pplay = pInfo->mutable_fight();
	net::fight_play_log* plog = pplay->add_logs();
	fightGameRecord& record = m_record;
	for (uint16 j = 0; j<JETTON_INDEX_COUNT; j++) {
		plog->add_seats_win(record.wins[j]);
	}
	plog->set_card(record.card);
